    ...
    return (
      <ShowView className = "class-view class-show">
        ...
        <p className="state-message">
          {isLoading ? 'Loading class Details ...' : isError ? 'Error' : 'No data'}</p>
      </ShowView>
    )
  }
...
const placeholderUrl = `https://placehold.co/600x400?text=${encodeURIComponent(teacherInitials || 'NA')}`
  const {name, description, status, capacity, bannerUrl, bannerCldPubId,
    subject, teacher, department}  = classDetails;
  return (
    <ShowView className = "class-view class-show">
      <ShowViewHeader resource = "classes" title = "Class Details" />
      <div className="banner">
        {bannerUrl?
          (<AdvancedImage alt = "ClassBanner" cldImg ={bannerPhoto(bannerCldPubId ?? '', name)}/>):
